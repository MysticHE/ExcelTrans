...
import {
  ...
  ChevronDown, ChevronRight, AlertTriangle, CheckCircle2, Sparkles, X
} from 'lucide-react';
import { getTemplate } from '../services/intelApi';
import { Card, Badge, Button, Alert, Input, cn } from '../ui';
...
function getSheetQualityBadge(sheet) {
...
function SheetCard({ sheet, selected, onSelect }) {
  ...
  const quality = getSheetQualityBadge(sheet);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        <Badge variant={isEmpty ? 'red' : 'gray'}>
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
      ...
      <div className="relative mb-3">
        ...
        <input
          className="w-full pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400"
    ...
    <div className="border border-amber-200 rounded-xl overflow-hidden">
      ...
      >
        ...
        <span className="text-xs text-amber-600 ml-1">
          ({onlyInA.length + onlyInB.length} differences)
        </span>
      ...
      {open && (
        <div className="p-4 bg-white grid grid-cols-2 gap-4 border-t border-amber-100">
          {onlyInA.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-red-700 mb-2">Only in File A ({onlyInA.length})</p>
          ...
          {onlyInB.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-green-700 mb-2">Only in File B ({onlyInB.length})</p>
              <div className="flex flex-wrap gap-1">
...
export default function Step2_SheetSelector({ wizard, aiConfig }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl px-4 py-3">
          ...
          <div className="flex gap-2 ml-4 shrink-0">
            <Button
              variant="secondary"
              size="sm"
              onClick={() => applyTemplate(suggestion.slug, 3)}
            ...
            <Button
              variant="primary"
              size="sm"
              onClick={() => applyTemplate(suggestion.slug, 4)}
              disabled={templateLoading}
      ...
      <div className={`grid gap-6 ${hasBothFiles ? 'md:grid-cols-2' : ''}`}>
        ...
        <div>
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
            ...
            {filteredSheetsA.length === 0 && (
              <p className="text-xs text-gray-400 italic p-2">No sheets match search.</p>
            )}
          </div>
        </div>
      ...
      {/* Column preview */}
      {selectedSheetA && (
        <ColumnPreviewPanel sheet={selectedSheetA} />
      )}
